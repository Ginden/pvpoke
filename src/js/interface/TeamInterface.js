    ...
    function createInstance() {
        ...
        function interfaceObject() {
            ...
            this.init = function () {
                ...
                if (!get) {
                    gm.loadRankingData(
                        self,
                        "overall",
                        parseInt($(".format-select option:selected").attr("value")),
                        "all",
                    );
                }
                    ...
                    if (get.hasOwnProperty(key)) {
                        ...
                        switch (key) {
                            ...
                            case "cp":
                                ...
                                $(
                                    '.format-select option[value="' + getCP + '"][cup="' + battle.getCup().name + '"]',
            ...
            this.updateTeamResults = function () {
                ...
                altRankings.sort((a, b) =>
                    a.matchupAltScore > b.matchupAltScore ? -1 : b.matchupAltScore > a.matchupAltScore ? 1 : 0,
                );
            ...
            this.displayAlternatives = function (list) {
                ...
                while (count < total && i < altRankings.length) {
                    ...
                    if (battle.getCup().name == "labyrinth") {
                        if (
                            excludedTypes.indexOf(pokemon.types[0]) > -1 ||
                            excludedTypes.indexOf(pokemon.types[1]) > -1
                        ) {
                            i++;
                            continue;
                        }
                    ...
                    if (battle.getCup().name == "continentals-3") {
                        ...
                        $row.find("th.name").append(
                            '<div class="region-label ' + regionName.toLowerCase() + '">Slot ' + slotNumber + "</div>",
                        );
                    }
                    ...
                    if (battle.getCup().slots) {
                        ...
                        for (var j = 0; j < slots.length; j++) {
                            if (
                                slots[j].pokemon.indexOf(pokemon.speciesId) > -1 ||
                                slots[j].pokemon.indexOf(pokemon.speciesId.replace("_shadow", "")) > -1
                            ) {
                                slot = j + 1;
                                break;
                            }
                ...
                $(".poke-search[context='alternative-search']")
                    .parent()
                    .css("max-width", $(".alternatives-table").width());
            ...
            function addAlternativePokemon(e) {
                ...
                if ($(e.target).attr("alias") != $(e.target).attr("pokemon")) {
                    ...
                    $('.modal .move-select.fast option[value="' + pokemon.fastMove.moveId + '"]').prop(
                        "selected",
                        "selected",
                    );
                    $(".modal .move-select.fast").trigger("change");

                    $(".modal .move-select.charged")
                        .eq(0)
                        .find('option[value="' + pokemon.chargedMoves[0].moveId + '"]')
                        .prop("selected", "selected");
                    $(".modal .move-select.charged").eq(0).trigger("change");

                    $(".modal .move-select.charged")
                        .eq(1)
                        .find('option[value="' + pokemon.chargedMoves[1].moveId + '"]')
                        .prop("selected", "selected");
