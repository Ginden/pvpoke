...
var pokeSearch = new (function () {
    ...
    $("body").on(
        "keyup",
        ".poke-search[context='ranking-search'], .poke-search[context='alternative-search']",
        function (e) {
        ...
        if ($(this).attr("context") != "pokeselect") {
            ...
            $(".modal a.nickname-list").click(function (e) {
                ...
                for (var i = 0; i < gm.data.pokemon.length; i++) {
                    ...
                    if (
                        pokemon.nicknames &&
                        !pokemon.speciesId.includes("shadow") &&
                        !pokemon.speciesId.includes("mega")
                    ) {
                        $(".modal .search-nicknames tbody").append(
                            "<tr><td>" +
                                pokemon.speciesName +
                                "</td><td>" +
                                pokemon.nicknames.join(", ") +
                                "</td></tr>",
                        );
                    }
                }
    ...
    $("a.search-traits").click(function (e) {
        ...
        for (var i = 0; i < traits.pros.length; i++) {
            $(".modal .traits").append(
                '<div class="pro" value="' + traits.pros[i] + '">+ ' + toTitleCase(traits.pros[i]) + "</div>",
            );
        }

        for (var i = 0; i < traits.cons.length; i++) {
            $(".modal .traits").append(
                '<div class="con" value="' + traits.cons[i] + '">- ' + toTitleCase(traits.cons[i]) + "</div>",
            );
        }
