...
function PokeSelect(element, i) {
    ...
    this.update = function () {
        if (selectedPokemon) {
            ...
            var effectiveOverall = Math.round(
                (selectedPokemon.stats.hp * selectedPokemon.getEffectiveStat(0) * selectedPokemon.getEffectiveStat(1)) /
                    1000,
            );
            ...
            for (var i = 0; i < selectedPokemon.types.length; i++) {
                ...
                if (selectedPokemon.types[i] != "none") {
                    $el.find(".types").append(
                        '<div class="type ' + selectedPokemon.types[i] + '">' + typeStr + "</div>",
                    );
                }
            }
                ...
                if (forms.length > 0) {
                    ...
                    if (forms.length > 1) {
                        ...
                        for (var i = 0; i < forms.length; i++) {
                            if (forms[i].speciesId != selectedPokemon.speciesId) {
                                $formSelect.append(
                                    "<option value='" + forms[i].speciesId + "'>" + forms[i].speciesName + "</option>",
                                );
                            }
                        }
                    } else {
    ...
    this.updateIVRank = function () {
        ...
        if (rank != 0) {
            ...
            $el.find(".iv-rank").css(
                "background",
                "rgba(" + rankColor[0] + "," + rankColor[1] + "," + rankColor[2] + ", 1)",
            );
        } else {
    ...
    this.generateIVTableResult = function (poke) {
        ...
        for (var i = 0; i < leagues.length; i++) {
            ...
            for (var n = 0; n < levelCaps.length; n++) {
                ...
                if (
                    leagues[i] - pokemon.cp < 300 ||
                    (pokemon.stats.atk * pokemon.stats.def * pokemon.stats.hp) / 100 > statProductMin
                ) {
                    rows.push({
                        cp: pokemon.cp,
                        level: pokemon.level,
    ...
    this.removePokemonFromOptions = function (pokemonList) {
        for (var i = 0; i < pokemonList.length; i++) {
            ...
            searchArr.splice(
                searchArr.findIndex((p) => p.speciesId == pokemonList[i].speciesId),
                1,
            );
        }
    };
    ...
    $el.find(".check.switch-delay").on("click", function (e) {
        ...
        selectedPokemon.startCooldown =
            selectedPokemon.startCooldown == 0
                ? (selectedPokemon.startCooldown = 1000)
                : (selectedPokemon.startCooldown = 0);
        selectedPokemon.isCustom = true;
        isCustom = true;
    });
    ...
    $el.on("mousemove", ".move-bar", function (e) {
        ...
        if (battle.getOpponent(selectedPokemon.index)) {
            ...
            displayDamage = battle.calculateDamageByStats(
                selectedPokemon,
                opponent,
                selectedPokemon.getEffectiveStat(0, true),
                opponent.getEffectiveStat(1, true),
                effectiveness,
                move,
            );
        }
        ...
        $tooltip
            ...
            .html(
                displayDamage +
                    " (" +
                    percent +
                    '%) <span class="label">dmg</span><br>' +
                    move.energy +
                    ' <span class="label">energy</span><br>' +
                    dpe +
    ...
    $el.find(".poke-search").keydown(function (e) {
        if (e.which === 38 || e.which === 40) {
            ...
            if (e.which == 38) {
                if (
                    $el.find(".poke-select option:selected").prev() &&
                    !$el.find(".poke-select option:selected").prev().prop("disabled")
                ) {
