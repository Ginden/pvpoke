    ...
    function createInstance() {
        ...
        function rankerObject() {
                ...
                for (var i = 0; i < rankCount; i++) {
                    ...
                    for (var n = 0; n < team.length; n++) {
                        ...
                        if (avgPokeRating > 500) {
                            alternativeScore = 500 + Math.pow(avgPokeRating - 500, 0.75);
                        ...
                        if (context == "matrix") {
                            ...
                            var breakpoint = battle.calculateDamageByStats(
                                pokemon,
                                opponent,
                                pokemon.getEffectiveStat(0),
                                opponent.getEffectiveStat(1),
                                opponent.typeEffectiveness[pokemon.fastMove.type],
                                pokemon.fastMove,
                            );

                            var bulkpoint = battle.calculateDamageByStats(
                                opponent,
                                pokemon,
                                opponent.getEffectiveStat(0),
                                pokemon.getEffectiveStat(1),
                                pokemon.typeEffectiveness[opponent.fastMove.type],
                                opponent.fastMove,
                            );
                            ...
                            if (pokemon.chargedMoves.length > 0) {
                                var breakpointCM1 = battle.calculateDamageByStats(
                                    pokemon,
                                    opponent,
                                    pokemon.getEffectiveStat(0),
                                    opponent.getEffectiveStat(1),
                                    opponent.typeEffectiveness[pokemon.chargedMoves[0].type],
                            ...
                            if (pokemon.chargedMoves.length > 1) {
                                var breakpointCM2 = battle.calculateDamageByStats(
                                    pokemon,
                                    opponent,
                                    pokemon.getEffectiveStat(0),
                                    opponent.getEffectiveStat(1),
                                    opponent.typeEffectiveness[pokemon.chargedMoves[1].type],
                            ...
                            if (opponent.chargedMoves.length > 0) {
                                var bulkpointCM1 = battle.calculateDamageByStats(
                                    opponent,
                                    pokemon,
                                    opponent.getEffectiveStat(0),
                                    pokemon.getEffectiveStat(1),
                                    pokemon.typeEffectiveness[opponent.chargedMoves[0].type],
                                    opponent.chargedMoves[0],
                                );
                                matchup.bulkpointCM1 = bulkpointCM1;
                            }

                            if (opponent.chargedMoves.length > 1) {
                                var bulkpointCM2 = battle.calculateDamageByStats(
                                    opponent,
                                    pokemon,
                                    opponent.getEffectiveStat(0),
                                    pokemon.getEffectiveStat(1),
                                    pokemon.typeEffectiveness[opponent.chargedMoves[1].type],
                                    opponent.chargedMoves[1],
                                );
            ...
            this.applySettingsToPokemon = function (settings, pokemon) {
                ...
                if (
                    settings.bait != defaultSettings.bait ||
                    settings.startCooldown != defaultSettings.startCooldown ||
                    settings.optimizeMoveTiming != defaultSettings.optimizeMoveTiming ||
                    settings.startStatBuffs != defaultSettings.startStatBuffs
                ) {
                    pokemon.isCustom = true;
                }
            };
