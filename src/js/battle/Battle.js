...
function Battle() {
    ...
    this.calculateDamageByStats = function (attacker, defender, attack, defense, effectiveness, move) {
        ...
        var damage =
            Math.floor(move.power * move.stab * (attack / defense) * effectiveness * 0.5 * bonusMultiplier) + 1;
    ...
    this.getTypeTraits = function (type) {
        ...
        switch (type) {
            case "normal":
                traits = { resistances: [], weaknesses: ["fighting"], immunities: ["ghost"] };
                break;
            ...
            case "electric":
                traits = { resistances: ["flying", "steel", "electric"], weaknesses: ["ground"], immunities: [] };
                break;

            case "psychic":
                traits = { resistances: ["fighting", "psychic"], weaknesses: ["bug", "ghost", "dark"], immunities: [] };
                break;

            case "ice":
                traits = { resistances: ["ice"], weaknesses: ["fighting", "fire", "steel", "rock"], immunities: [] };
                break;
    ...
    this.step = function () {
        ...
        turnActions.sort((a, b) =>
            a.settings.priority > b.settings.priority ? -1 : b.settings.priority > a.settings.priority ? 1 : 0,
        );
    ...
    this.decideAction = function (poke, opponent) {
        ...
        for (var n = 0; n < poke.activeChargedMoves.length; n++) {
            ...
            if (poke.energy >= poke.activeChargedMoves[n].energy) {
                ...
                chargedMoveReady.push(
                    (Math.ceil((poke.activeChargedMoves[n].energy - poke.energy) / poke.fastMove.energyGain) *
                        poke.fastMove.cooldown) /
                        500,
                );
            }
        }
                    ...
                    if (currState.opEnergy >= opponent.activeChargedMoves[n].energy) {
                        ...
                        if (moveDamage >= currState.hp) {
                            ...
                            if (
                                ...
                                opponent.fastMove.cooldown % poke.fastMove.cooldown == 0
                            ) {
                                turnsToLive++;
                            }
                            ...
                            self.logDecision(
                                turns,
                                poke,
                                " opponent has energy to use " +
                                    opponent.activeChargedMoves[n].name +
                                    " and it would do " +
                                    moveDamage +
                                    " damage. I have " +
                                    turnsToLive +
                                    " turn(s) to live, opponent has " +
                                    currState.opEnergy,
                            );
                            break;
                        }
            ...
            if (
                ...
                if (prevMoveDamage == -1) {
                    ...
                    self.logDecision(
                        turns,
                        poke,
                        " uses a fast move because it has " +
                            turnsToLive +
                            " turn(s) before it is KO'd but has no energy.",
                    );
                    return;
                    ...
                    self.logDecision(
                        ...
                        poke,
                        " uses " +
                            poke.chargedMoves[maxDamageMoveIndex].name +
                            " because it has " +
                            turnsToLive +
                            " turn(s) before it is KO'd.",
                    );

                    action = new TimelineAction("charged", poke.index, turns, maxDamageMoveIndex, {
                        shielded: false,
                        buffs: false,
        ...
        if (poke.optimizeMoveTiming) {
            ...
            if ((opponent.cooldown == 0 || opponent.cooldown > targetCooldown) && targetCooldown > 0) {
                ...
                var turnsPlanned =
                    poke.fastMove.cooldown / 500 + Math.floor(poke.energy / poke.activeChargedMoves[0].energy);
                ...
                if (opponent.shields == 0) {
                    for (var n = 0; n < poke.activeChargedMoves.length; n++) {
                        poke.activeChargedMoves[n].damage = self.calculateDamage(
                            poke,
                            opponent,
                            poke.activeChargedMoves[n],
                        );

                        if (
                            poke.energy >= poke.activeChargedMoves[n].energy &&
                            poke.activeChargedMoves[n].damage >= opponent.hp
                        ) {
                            optimizeTiming = false;
                            break;
                        }
                ...
                for (var n = 0; n < opponent.activeChargedMoves.length; n++) {
                    var fastMovesFromCharged = Math.ceil(
                        (opponent.activeChargedMoves[n].energy - opponent.energy) / opponent.fastMove.energyGain,
                    );
                    ...
                    opponent.activeChargedMoves[n].damage = self.calculateDamage(
                        opponent,
                        poke,
                        opponent.activeChargedMoves[n],
                    );

                    var moveDamage =
                        opponent.activeChargedMoves[n].damage + opponent.fastMove.damage * fastMovesInFastMove;
        ...
        var bestCycleDamage =
            bestChargedDamage + fastDamage * Math.ceil(poke.bestChargedMove.energy / poke.fastMove.energyGain);
        var minimumCycleThreshold = 2;

        // Prefer non-debuffing moves when it will take multiple to KO
        if (
            poke.bestChargedMove.selfDebuffing &&
            poke.bestChargedMove.energy > poke.fastestChargedMove.energy &&
            poke.bestChargedMove.dpe / poke.fastestChargedMove.dpe < 2
        ) {
        ...
        if (opponent.hp / bestCycleDamage > minimumCycleThreshold) {
            ...
            if (poke.activeChargedMoves.length > 1) {
                if (
                    ...
                    self.wouldShield(poke, opponent, poke.activeChargedMoves[1]).value
                ) {
                ...
                if (poke.bestChargedMove.selfDebuffing) {
                    for (var i = 0; i < poke.activeChargedMoves.length; i++) {
                        if (
                            !poke.activeChargedMoves[i].selfDebuffing &&
                            selectedMove.dpe / poke.activeChargedMoves[i].dpe < 2
                        ) {
            ...
            if (poke.energy < selectedMove.energy) {
                ...
                if (selectedMove.selfDebuffing) {
                    var energyToReach =
                        poke.energy +
                        Math.floor((100 - poke.energy) / poke.fastMove.energyGain) * poke.fastMove.energyGain;
                ...
                action = new TimelineAction("charged", poke.index, turns, poke.chargedMoves.indexOf(selectedMove), {
                    shielded: false,
                    buffs: false,
                    priority: poke.priority,
                });
        ...
        while (DPQueue.length != 0) {
            ...
            for (var n = 0; n < poke.activeChargedMoves.length; n++) {
                if (currState.energy >= poke.activeChargedMoves[n].energy) {
                    ...
                    DPchargedMoveReady.push(
                        (Math.ceil((poke.activeChargedMoves[n].energy - currState.energy) / poke.fastMove.energyGain) *
                            poke.fastMove.cooldown) /
                            500,
                    );
                }
            ...
            for (var n = 0; n < poke.activeChargedMoves.length; n++) {
                ...
                if (hasNonDebuff && poke.speciesName == "Melmetal" && opponent.speciesName == "Cresselia") {
                    if (
                        poke.activeChargedMoves[n].selfDebuffing &&
                        poke.activeChargedMoves[n].buffs[1] < 1 &&
                        opponent.hp > moveDamage * (1 + 4 / (4 - poke.activeChargedMoves[n].buffs[0]))
                    ) {
                        continue;
                    }
                }
                ...
                if (DPQueue.length == 0) {
                    DPQueue.unshift(
                        new BattleState(
                            ...
                            0,
                            currState.turn + (movesToFarmDown * poke.fastMove.cooldown) / 500,
                            currState.opponentShields,
                            currState.moves,
                            currState.buffs,
                            currState.chance,
                        ),
                ...
                if (DPchargedMoveReady[n] == 0) {
                        ...
                        if (DPQueue[i].oppHealth == newOppHealth && DPQueue[i].buffs == attackMult) {
                            if (DPQueue[i].energy == currState.energy - poke.activeChargedMoves[n].energy) {
                                ...
                                for (var x = 0; x < DPQueue[i].moves.length; x++) {
                                    ...
                                    if (
                                        DPQueue[i].moves[x].buffApplyChance == 1 &&
                                        DPQueue[i].moves[x].buffTarget == "self" &&
                                        DPQueue[i].moves[x].buffs[0] + DPQueue[i].moves[x].buffs[1] > 0
                                    ) {
                                        DPDebuffs--;
                                    }
                                }
                                ...
                                for (var x = 0; x < tempState.length; x++) {
                                    ...
                                    if (
                                        tempState[x].buffApplyChance == 1 &&
                                        tempState[x].buffTarget == "self" &&
                                        tempState[x].buffs[0] + tempState[x].buffs[1] > 0
                                    ) {
                                        currDebuffs--;
                                    }
                                }
                        ...
                        if (DPQueue.length == 0) {
                            DPQueue.unshift(
                                new BattleState(
                                    ...
                                    currState.turn + 1,
                                    newShields,
                                    currState.moves.concat([poke.activeChargedMoves[n]]),
                                    attackMult,
                                    currState.chance,
                                ),
                            );
                            ...
                            if (changeTTKChance != 0) {
                                DPQueue.unshift(
                                    new BattleState(
                                        ...
                                        currState.moves.concat([poke.activeChargedMoves[n]]),
                                        possibleAttackMult,
                                        currState.chance * changeTTKChance,
                                    ),
                                );
                            }
                        } else {
                            while (DPQueue[i].turn <= currState.turn + 1) {
                                if (
                                    DPQueue[i].hp <= newOppHealth &&
                                    DPQueue[i].energy >= newEnergy &&
                                    DPQueue[i].buffs >= attackMult &&
                                    DPQueue[i].shields <= newShields
                                ) {
                                    insert = false;
                                    break;
                                }
                            ...
                            if (insert) {
                                DPQueue.splice(
                                    ...
                                    0,
                                    new BattleState(
                                        currState.energy - poke.activeChargedMoves[n].energy,
                                        newOppHealth,
                                        currState.turn + 1,
                                        newShields,
                                        currState.moves.concat([poke.activeChargedMoves[n]]),
                    ...
                    if (
                        poke.activeChargedMoves[n].selfDebuffing &&
                        poke.activeChargedMoves[n].buffs[0] < 0 &&
                        poke.activeChargedMoves[n].energy * 2 <= 100
                    ) {
                        var newTurn =
                            (Math.ceil(
                                (poke.activeChargedMoves[n].energy * 2 - currState.energy) / poke.fastMove.energyGain,
                            ) *
                                poke.fastMove.cooldown) /
                            500;
                        newEnergy =
                            Math.floor(newTurn / (poke.fastMove.cooldown / 500)) * poke.fastMove.energyGain +
                            currState.energy -
                            poke.activeChargedMoves[n].energy;

                        if (newTurn != 0) {
                            // Calculate new health
                            newOppHealth =
                                currState.oppHealth - fastSimulatedDamage * (newTurn / (poke.fastMove.cooldown / 500));
                    ...
                    var newEnergy =
                        currState.energy -
                        poke.activeChargedMoves[n].energy +
                        poke.fastMove.energyGain * (DPchargedMoveReady[n] / (poke.fastMove.cooldown / 500));
                    var newOppHealth =
                        currState.oppHealth -
                        moveDamage -
                        fastSimulatedDamage * (DPchargedMoveReady[n] / (poke.fastMove.cooldown / 500));
        ...
        if (poke.baitShields && opponent.shields > 0 && poke.activeChargedMoves.length > 1) {
            if (
                poke.energy < poke.activeChargedMoves[1].energy &&
                poke.activeChargedMoves[1].dpe > finalState.moves[0].dpe
            ) {
                var bait = true;

                // Don't go for baits if you have an effective self buffing move
                if (
                    poke.activeChargedMoves[1].dpe / poke.activeChargedMoves[0].dpe <= 1.5 &&
                    poke.activeChargedMoves[0].selfBuffing
                ) {
                    bait = false;
                }
                ...
                if (bait) {
                    self.logDecision(
                        turns,
                        poke,
                        " doesn't use " + finalState.moves[0].name + " because it wants to bait",
                    );
                    useChargedMove = false;
                    return;
                }
        ...
        if (poke.baitShields && opponent.shields > 0 && poke.activeChargedMoves.length > 1) {
            var dpeRatio =
                poke.activeChargedMoves[1].damage /
                poke.activeChargedMoves[1].energy /
                (finalState.moves[0].damage / finalState.moves[0].energy);
        ...
        if (
            opponent.shields > 0 &&
            poke.activeChargedMoves.length > 1 &&
        ...
        if (
            opponent.shields == 0 &&
            poke.activeChargedMoves.length > 1 &&
            finalState.moves[0].selfDebuffing &&
            finalState.moves[0].energy > 50 &&
            poke.hp / poke.stats.hp > 0.5 &&
            finalState.moves[0].damage / opponent.hp < 0.8
        ) {
            finalState.moves[0] = poke.activeChargedMoves[0];
        }
        ...
        if (
            poke.activeChargedMoves.length > 1 &&
            poke.activeChargedMoves[0].energy - finalState.moves[0].energy <= 5 &&
            poke.activeChargedMoves[0].dpe > finalState.moves[0].dpe &&
            poke.activeChargedMoves[0].selfBuffing
        ) {
            finalState.moves[0] = poke.activeChargedMoves[0];
        ...
        if (opponent.shields > 0 && poke.activeChargedMoves.length > 1) {
            ...
            if (poke.activeChargedMoves[0].selfDebuffing && !poke.activeChargedMoves[1].selfBuffing) {
                ...
                if (poke.baitShields || opponent.hp - poke.activeChargedMoves[0].damage > 10) {
                    ...
                    if (
                        poke.activeChargedMoves[1].energy - poke.activeChargedMoves[0].energy <= 10 &&
        ...
        if (finalState.moves[0].selfDebuffing && poke.shields == 0 && poke.energy < 100 && opponent.bestChargedMove) {
            if (
                ...
                !self.wouldShield(opponent, poke, opponent.bestChargedMove).value &&
                !poke.activeChargedMoves[0].selfBuffing
            ) {
                useChargedMove = false;
                self.logDecision(
                    turns,
                    poke,
                    " is deferring its self debuffing move until after the opponent fires its move",
                );
                return;
            }
        }
        ...
        if (finalState.moves[0].selfDebuffing) {
            ...
            if (poke.energy < targetEnergy) {
                ...
                if (
                    poke.activeChargedMoves[0].selfBuffing ||
                    self.wouldShield(poke, opponent, finalState.moves[0]).value
                ) {
                    finalState.moves[0] = poke.activeChargedMoves[0];
                }
            }
        ...
        if (poke.energy >= finalState.moves[0].energy) {
            if (finalState.moves.length > 1) {
                self.logDecision(
                    ...
                    " uses " +
                        finalState.moves[0].name +
                        " because it thinks that using " +
                        (finalState.moves.length - 1) +
                        " moves afterwards is the best plan.",
                );
                ...
                for (var i = 1; i < finalState.moves.length; i++) {
                    self.logDecision(
                        turns,
                        poke,
                        " wants to use " + finalState.moves[i].name + " after it uses " + finalState.moves[i - 1].name,
                    );
                }
            } else {
                self.logDecision(
                    ...
                    " uses " +
                        ...
                        " at turn " +
                        turns +
                        " because it KO's or it wants to farm down afterwards",
                );
            }
        } else {
            useChargedMove = false;
        ...
        action = new TimelineAction("charged", poke.index, turns, poke.chargedMoves.indexOf(finalState.moves[0]), {
            shielded: false,
            buffs: false,
            priority: poke.priority,
        });
    ...
    this.decideRandomAction = function (poke, opponent) {
        ...
        for (var i = 0; i < poke.activeChargedMoves.length; i++) {
            if (poke.energy >= poke.activeChargedMoves[i].energy) {
                ...
                if (
                    i > 0 &&
                    poke.activeChargedMoves[i].damage < poke.activeChargedMoves[0].damage &&
                    poke.activeChargedMoves[i].energy >= poke.activeChargedMoves[0].energy &&
                    !poke.activeChargedMoves[i].selfBuffing
                ) {
                    chargedMoveWeight = 0;
                }
                ...
                chargedMoveValues.push({
                    move: poke.activeChargedMoves[i],
                    damage: damage,
                    weight: chargedMoveWeight,
                    index: i,
                });
            }
        }
        ...
        if (chargedMoveValues.length > 1) {
            ...
            if (
                chargedMoveValues[0].damage >= opponent.hp &&
                chargedMoveValues[1].damage >= opponent.hp &&
                opponent.shields > 0
            ) {
                if (
                    chargedMoveValues[0].move.selfDebuffing &&
                    !chargedMoveValues[1].move.selfDebuffing &&
                    chargedMoveValues[1].move.energy <= chargedMoveValues[0].move.energy
                ) {
                    chargedMoveValues[0].weight = 0;
                } else if (
                    chargedMoveValues[1].move.selfDebuffing &&
                    !chargedMoveValues[0].move.selfDebuffing &&
                    chargedMoveValues[0].move.energy <= chargedMoveValues[1].move.energy
                ) {
        ...
        for (var i = 0; i < chargedMoveValues.length; i++) {
            actionOptions.push(
                new DecisionOption("CHARGED_MOVE_" + chargedMoveValues[i].index, chargedMoveValues[i].weight),
            );
        }
                ...
                if (poke.energy >= move.energy) {
                    if (mode == "simulate") {
                        ...
                        for (var i = 0; i < turnActions.length; i++) {
                            if (
                                (turnActions[i].type == "charged" || turnActions[i].type == "fast") &&
    ...
    this.useMove = function (attacker, defender, move, forceShields, forceBuff, charge) {
        ...
        if (move.energy > 0) {
            ...
            for (var i = 0; i < 8; i++) {
                timeline.push(
                    new TimelineEvent("tap " + move.type, "Swipe", attacker.index, time + 1000 * i, turns, [i]),
                );
            }
            ...
            if ((sandbox && forceShields && defender.shields > 0) || (!sandbox && defender.shields > 0)) {
                ...
                if (!sandbox && move.buffs && move.selfBuffing) {
                    if ((move.buffTarget == "self" && move.buffs[0] > 0) || move.buffTarget == "opponent") {
                ...
                if (!sandbox && defender.bestChargedMove && defender.bestChargedMove.selfDefenseDebuffing) {
                    if (attacker.shields > 0) {
                        ...
                        var fastToNextCharged = Math.ceil(
                        ...
                        var cycleDamage =
                            fastToNextCharged * defender.fastMove.damage + defender.bestChargedMove.damage;

                        var attackerTurnsToNextCharged =
                            Math.ceil(
                                (attacker.activeChargedMoves[0].energy - attacker.energy) /
                                    attacker.fastMove.energyGain,
                            ) *
                            (attacker.fastMove.cooldown / 500);
                ...
                if (useShield) {
                    ...
                    timeline.push(
                        new TimelineEvent("shield", "Shield", defender.index, time + 8500, turns, [damageBlocked]),
                    );
                    damage = 1;
                    defender.shields--;
                    ...
                    if (mode == "emulate") {
                        ...
                        for (var n = buffs.length - 1; n >= 0; n--) {
                            if (buffs[n] != 0) {
                                ...
                                turnMessages.push({
                                    index: buffTargets[i].target.index,
                                    str: statNames[n] + " " + statDescription + "!",
                                });
                            }
                        }
                    }
        ...
        if (attacker.formChange) {
            if (
                attacker.formChange.trigger == "charged_move" &&
                move.energy > 0 &&
    ...
    this.wouldShield = function (attacker, defender, move) {
        ...
        var fastAttacks =
            Math.ceil((move.energy - Math.max(attacker.energy - move.energy, 0)) / attacker.fastMove.energyGain) + 1; // Give some margin for error here
    ...
    this.calculateTurnMargin = function () {
        ...
        for (var i = 0; i < subject.chargedMoves.length; i++) {
            ...
            var fastMovesFromChargedMove = Math.ceil(
