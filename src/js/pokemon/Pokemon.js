...
function Pokemon(id, i, b) {
    ...
    var cpms = [
        ...
        0.236572655026622, 0.255720049142837, 0.273530381100769, 0.29024988412857, 0.306057381335773, 0.321087598800659,
        0.335445032295077, 0.349212676286697, 0.36245774877879, 0.375235587358474, 0.387592411085168, 0.399567276239395,
        0.41119354951725, 0.422500014305114, 0.432926413410414, 0.443107545375824, 0.453059953871985, 0.46279838681221,
        0.472336077786704, 0.481684952974319, 0.490855810259008, 0.499858438968658, 0.508701756943992,
        0.517393946647644, 0.525942508771329, 0.534354329109191, 0.542635762230353, 0.550792694091796,
        0.558830599438087, 0.566754519939422, 0.574569148039264, 0.582278907299041, 0.589887911977272, 0.59740000963211,
        0.604823657502073, 0.61215728521347, 0.61940411056605, 0.626567125320434, 0.633649181622743, 0.640652954578399,
        ...
        0.752729105305821, 0.75568550825119, 0.758630366519684, 0.761563837528228, 0.764486065255226, 0.767397165298461,
        0.77029727397159, 0.77318650484085, 0.776064945942412, 0.778932750225067, 0.781790064808426, 0.784636974334716,
        0.787473583646825, 0.790300011634826, 0.792803950958807, 0.795300006866455, 0.79780392148697, 0.800300002098083,
        0.802803892322847, 0.805299997329711, 0.807803863460723, 0.81029999256134, 0.812803834895026, 0.815299987792968,
        0.817803806620319, 0.820299983024597, 0.822803778631297, 0.825299978256225, 0.827803750922782,
        0.830299973487854, 0.832803753381377, 0.835300028324127, 0.837803755931569, 0.840300023555755,
        0.842803729034748, 0.845300018787384, 0.847803702398935, 0.850300014019012, 0.852803676019539, 0.85530000925064,
    ...
    this.generateIVCombinations = function (sortStat, sortDirection, resultCount, filters, ivFloor) {
        ...
        if (
            (self.hasTag("legendary") || self.hasTag("ultrabeast")) &&
            (self.shadowType == "shadow" || self.hasTag("shadow"))
        ) {
            floor = 6;
        }
        ...
        while (hpIV >= floor) {
            ...
            while (defIV >= floor) {
                ...
                while (atkIV >= floor) {
                    if (targetCP > 500) {
                        // Ignore level floor for Little Cup right now
        ...
        combinations.sort((a, b) =>
            a[sortStat] > b[sortStat] ? -1 * sortDirection : b[sortStat] > a[sortStat] ? 1 * sortDirection : 0,
        );
    ...
    this.getIVRank = function (sortStat) {
        ...
        var rank = combinations.findIndex(
            (combo) => combo.ivs.atk == this.ivs.atk && combo.ivs.def == this.ivs.def && combo.ivs.hp == this.ivs.hp,
        );
        rank++;
    ...
    this.calculateBreakpoints = function (defender, move) {
        ...
        var minDamage = battle.calculateDamageByStats(
            ...
            defender,
            minAttack,
            defender.stats.def * defender.shadowDefMult * defenseStatMultiplier,
            effectiveness,
            move,
        );
        ...
        for (var i = minDamage; i <= maxDamage; i++) {
            ...
            var maxDefenseBreakpoint = battle.calculateBreakpoint(
                ...
                defender,
                i,
                maxDefense * defender.shadowDefMult * defenseStatMultiplier,
                effectiveness,
                move,
            );
    ...
    this.calculateBulkpoints = function (attacker, move) {
        ...
        var minDamage = battle.calculateDamageByStats(
            attacker,
            self,
            attacker.stats.atk * attacker.shadowAtkMult * attackStatMultiplier,
            maxDefense,
            effectiveness,
            move,
        );
        ...
        for (var i = minDamage; i <= maxDamage; i++) {
            ...
            var maxAttackBulkpoint = battle.calculateBulkpoint(
                ...
                self,
                i,
                maxAttack * attacker.shadowAtkMult * attackStatMultiplier,
                effectiveness,
                move,
            );
        ...
        if (this.chargedMoves.length > 0) {
            for (var i = 0; i < self.chargedMoves.length; i++) {
                ...
                if (self.chargedMoves[i].buffs && self.chargedMoves[i].buffApplyChance < 1) {
                    ...
                    if (self.chargedMoves[i].buffApplyChance == 0.5) {
            ...
            if (self.activeChargedMoves.length > 1) {
                ...
                if (
                    ...
                    if (
                        ...
                        self.activeChargedMoves[1].damage > self.activeChargedMoves[0].damage
                    ) {
                ...
                if (
                    self.activeChargedMoves[1].energy == self.activeChargedMoves[0].energy &&
                    self.activeChargedMoves[0].buffs &&
                    self.activeChargedMoves[1].buffs &&
                    !self.activeChargedMoves[1].selfDebuffing &&
                    self.activeChargedMoves[0].buffs &&
                    self.activeChargedMoves[1].buffApplyChance > self.activeChargedMoves[0].buffApplyChance
                ) {
                    var move = self.activeChargedMoves[0];
                    self.activeChargedMoves.splice(0, 1);
                    self.activeChargedMoves.push(move);
                ...
                if (
                    self.activeChargedMoves[0].moveId == "FOCUS_BLAST" &&
                    self.activeChargedMoves[1].moveId == "ZAP_CANNON"
                ) {
                ...
                if (
                    ...
                    if (
                        self.activeChargedMoves[1].selfBuffing &&
                        self.activeChargedMoves[0].dpe - self.activeChargedMoves[1].dpe < 0.3
                    ) {
                        var move = self.activeChargedMoves[0];
                        self.activeChargedMoves.splice(0, 1);
                        self.activeChargedMoves.push(move);
                ...
                if (
                    self.activeChargedMoves[1].energy - self.activeChargedMoves[0].energy <= 10 &&
                    self.activeChargedMoves[0].selfDebuffing &&
                    self.activeChargedMoves[0].energy > 50 &&
                    !self.activeChargedMoves[1].selfDebuffing
                ) {
                    var move = self.activeChargedMoves[0];
                    self.activeChargedMoves.splice(0, 1);
                ...
                if (
                    self.activeChargedMoves[1].energy - self.activeChargedMoves[0].energy <= 5 &&
                    self.activeChargedMoves[1].selfBuffing
                ) {
                    var move = self.activeChargedMoves[0];
                    self.activeChargedMoves.splice(0, 1);
            ...
            for (var i = 0; i < self.activeChargedMoves.length; i++) {
                ...
                if (
                    ...
                    if (
                        !self.bestChargedMove.selfBuffing ||
                        (self.bestChargedMove.selfBuffing && move.dpe - self.bestChargedMove.dpe > 0.3)
                    ) {
            ...
            // Favor Obstruct over close energy moves
            if (
                self.activeChargedMoves[0].moveId == "OBSTRUCT" &&
                self.activeChargedMoves[0].energy - self.bestChargedMove.energy <= 5 &&
                self.activeChargedMoves[0].dpe / self.bestChargedMove.dpe > 0.2
            ) {
                self.bestChargedMove = self.activeChargedMoves[0];
            }
        } else {
    ...
    this.generateTraits = function () {
        ...
        var activationSpeed =
            Math.ceil((self.fastestChargedMove.energy * 2) / self.fastMove.energyGain) *
            self.fastMove.cooldown *
            (1 / 1000); // Avg speed over two cycles to account for overflow energy
        ...
        for (var i = 0; i < types.length; i++) {
            ...
            for (var n = 0; n < self.chargedMoves.length; n++) {
                var effectiveness = battle.getEffectiveness(self.chargedMoves[n].type, [
                    types[i].toLowerCase(),
                    "none",
                ]);
                var effectivePower =
                    self.chargedMoves[n].power *
                    self.chargedMoves[n].stab *
                    self.shadowAtkMult *
                    effectiveness *
                    (self.stats.atk / targetDef);
                var bestChargedMoveSpeed =
                    Math.ceil(self.chargedMoves[n].energy / self.fastMove.energyGain) * (self.fastMove.cooldown / 500);
        ...
        if (
            (self.chargedMoves.length == 1 ||
                (self.chargedMoves.length == 2 && self.chargedMoves[0].type == self.chargedMoves[1].type)) &&
            !inflexible
        ) {
            cons.push({
                trait: "Inflexible",
        ...
        if (r) {
            if (
                (r.scores[2] >= 90 || r.scores[3] >= 90) &&
                self.fastMove.energyGain / self.fastMove.cooldown >= 3 / 500
            ) {
                pros.push({
                    trait: "Dynamic",
        ...
        var effectiveDPT =
            (self.fastMove.power * self.fastMove.stab * self.shadowAtkMult * (self.stats.atk / targetDef)) /
        ...
        var effectivePower =
            self.bestChargedMove.power * self.bestChargedMove.stab * self.shadowAtkMult * (self.stats.atk / targetDef);
        var bestChargedMoveSpeed =
            Math.ceil(self.bestChargedMove.energy / self.fastMove.energyGain) * (self.fastMove.cooldown / 500);
        effectivePower = effectivePower * (30 / bestChargedMoveSpeed);
        ...
        if (
            self.hasMove("OCTAZOOKA") ||
            self.hasMove("LEAF_TORNADO") ||
            self.hasMove("MIRROR_SHOT") ||
            self.hasMove("MUDDY_WATER") ||
            self.hasMove("TRI_ATTACK")
        ) {
            cons.push({
                trait: "Chaotic",
        ...
        if (
            self.hasMove("BUBBLE_BEAM") ||
            self.hasMove("ICY_WIND") ||
            self.hasMove("LUNGE") ||
            self.hasMove("SAND_TOMB") ||
            self.hasMove("ACID_SPRAY") ||
    ...
    this.generateSimilarPokemon = function (traits) {
        ...
        pokemonList.sort((a, b) =>
            a.similarityScore > b.similarityScore ? -1 : b.similarityScore > a.similarityScore ? 1 : 0,
        );
    ...
    this.getBoostMove = function () {
        ...
        for (var i = 0; i < self.chargedMoves.length; i++) {
            if (
                ...
                self.chargedMoves[i].buffApplyChance >= 0.5 &&
                !self.chargedMoves[i].selfDebuffing
            ) {
    ...
    this.getBattleRating = function () {
        ...
        return Math.floor(
            500 * ((opponent.stats.hp - opponent.hp) / opponent.stats.hp) + 500 * (self.hp / self.stats.hp),
        );
    };
    ...
    this.calculateConsistency = function () {
        ...
        if (chargedMoves.length == 2) {
            ...
            for (var n = 0; n < effectivenessScenarios.length; n++) {
                ...
                for (var i = 0; i < chargedMoves.length; i++) {
                    if (
                        chargedMoves[i].buffs &&
                        chargedMoves[i].buffApplyChance < 1 &&
                        chargedMoves[i].buffApplyChance > 0.15
                    ) {
    ...
    this.getSlot = function (cup) {
        ...
        for (var j = 0; j < cup.slots.length; j++) {
            if (
                cup.slots[j].pokemon.indexOf(self.speciesId) > -1 ||
                cup.slots[j].pokemon.indexOf(self.speciesId.replace("_shadow", "")) > -1
            ) {
                slotNumber = j;
                break;
            }
    ...
    this.changeForm = function (id) {
        ...
        if (this.formChange.type == "toggle") {
            formId =
                this.activeFormId == this.formChange.defaultFormId
                    ? this.formChange.alternativeFormId
                    : this.formChange.defaultFormId;
        }
