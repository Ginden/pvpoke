...
function TeraRanker() {
    ...
    this.scoreOffense = function (raidBoss, attacker) {
        ...
        let attackerType1Effectiveness =
            self.getEffectivenessTraits(attacker.types[0], raidBoss.tera, attacker, raidBoss) *
            self.getStab(attacker, attacker.types[0], false);

        // The Pokemon will use its most effective offensive type, or at worst a neutral non-STAB move
        if (attacker.types.length > 1) {
            let attackerType2Effectiveness =
                self.getEffectivenessTraits(attacker.types[1], raidBoss.tera, attacker, raidBoss) *
                self.getStab(attacker, attacker.types[1], false);
